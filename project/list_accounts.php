...
<div class="list">
    ...
    $res = [];
    $db = getDB();
    $stmt = $db->prepare("SELECT id, account_number, user_id, account_type, opened_date, last_updated, balance, apy from Accounts WHERE active = 'Active' AND user_id = :q ORDER BY opened_date  DESC LIMIT 5");
    $r = $stmt->execute([":q" => "$q"]);
    if ($r) {
